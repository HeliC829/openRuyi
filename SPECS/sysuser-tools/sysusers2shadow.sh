	...
	do
		...
		g)
			...
			if ! /usr/bin/getent group "$1" >> /dev/null; then
				if [ -x "/usr/bin/groupadd" ]; then
					run /usr/bin/groupadd -r $ARGUMENTS
				elif [ -x "$busybox" ]; then
		...
		u)
			...
			if [ "${5:--}" != "-" ]; then
				...
				ARGUMENTS="$ARGUMENTS -s /usr/bin/nologin"
			fi
			...
			if [ -x /usr/bin/useradd ]; then
			    ...
			    run /usr/bin/useradd -r -c "$3" -d "${homedir}" $ARGUMENTS
			elif [ -x "$busybox" ]; then
			    if [ -n "$GROUP_ID" ] && [ "$GROUP_ID" != "-" ]; then
		...
		m)
			shift
			if [ -x /usr/bin/usermod ] ; then
				run /usr/bin/usermod -a -G "$2" "$1"
			elif [ -x "$busybox" ]; then
